...
$check_give_hit = checkGiveHit($bo_table, $view['wr_id'], $member['mb_id']);
$check_member_level = checkMemberLevel($view['wr_8']); ?>
?>
...
<div class="content view community">
    ...
    <div class="view_top">
        ...
        <p class="date">조회수 <?php echo $view['wr_good'] ?></p>
        <span class="time"><?php echo $view['datetime']?></span>
    </div>
    ...
    <div class="btn_group">
        <?php if($check_give_hit === '' && ($member['mb_id'] != $view['mb_id']) ){?>
            <a href="<?php echo $good_href.'&amp;'.$qstr ?>" class="btn_like"><?php echo number_format($view['wr_good']) ?></a>
        <?php } else if($check_give_hit === 'heart' || ($member['mb_id'] == $view['mb_id'])){?>
            <p class="btn_like heart"><?php echo number_format($view['wr_good']) ?></p>
        <?php } ?>
...
<section>
    ...
    <div class="layer layer_report is-hidden">
        <div class="inner">
            ...
            <div class="btn_group">
                <button type="button" class="button color yellow" onclick="location.href='<?php echo G5_THEME_URL.'/skin/board/general/update_report.php?bo_table=general&postid='.$view['wr_id'].'&memberid='.$member['mb_id'].'&reason='.'불건전'?>'">불건전</button>
                <button type="button" class="button color yellow" onclick="location.href='<?php echo G5_THEME_URL.'/skin/board/general/update_report.php?bo_table=general&postid='.$view['wr_id'].'&memberid='.$member['mb_id'].'&reason='.'홍보성'?>'">홍보성</button>
                <button type="button" class="button color yellow" onclick="location.href='<?php echo G5_THEME_URL.'/skin/board/general/update_report.php?bo_table=general&postid='.$view['wr_id'].'&memberid='.$member['mb_id'].'&reason='.'기타'?>'">기타</button>
