    ...
    while ($row = sql_fetch_array($result)) { // 제목글 뽑아서 링크 문자열 만들기..
        ...
        for ($i = $start_day; $i <= $end_day; $i++) {
            ...
            if ($member['mb_level'] < $board['bo_read_level']) {
                ...
                $html_day[$i].= "<div class='list_day'><i class='fa fa-map-marker'></i><a href='".G5_ADMIN_URL."/bbs/board.php?bo_table=$bo_table&year=$year&month=$month&wr_id=$row[wr_id]' id='subject_".$j."' ".$showLayer.">".$row['wr_subject']."</a></div>";
            }
    ?>
    <!-- 뷰 팝업레이어 -->
