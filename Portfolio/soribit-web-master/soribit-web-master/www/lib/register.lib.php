...
{
    if (preg_match("/^[_\.0-9a-zA-Z-]+@([0-9a-zA-Z][0-9a-zA-Z-]+\.)+[a-zA-Z]{2,6}$/i", $reg_mb_id) == false)
        return "올바른 아이디(이메일)을 입력해주세요.";
    else
        return "";
}
...
function valid_mb_nick($reg_mb_nick)
{
    if (!check_string($reg_mb_nick, G5_HANGUL + G5_ALPHABETIC + G5_NUMERIC))
        return "이름은 공백없이 한글, 영문, 숫자만 입력 가능합니다.";
    else
        return "";
}
...
function count_mb_nick($reg_mb_nick)
{
    if (strlen($reg_mb_nick) < 4)
        return "이름은 한글 2글자, 영문 4글자 이상 입력 가능합니다.";
    else
        return "";
}
...
function reserve_mb_nick($reg_mb_nick)
{
    ...
    if (preg_match("/[\,]?".preg_quote($reg_mb_nick)."/i", $config['cf_prohibit_id']))
        return "이미 예약된 단어로 사용할 수 없는 이름 입니다.";
    else
        return "";
}
...
function apiMbId($reg_mb_id)
{
    ...
    try{
        ...
        if ($reg_mb_id==""){
            $result['status'] = 200;
            $result['data']['code'] = 1;
            $result['data']['msg'] = "아이디(이메일)을 입력해 주십시오.";
            return $result;
        }
        $sql = " select count(*) as cnt from `{$g5['member_table']}` where mb_id = '$reg_mb_id' ";
        $row = sql_fetch($sql);
        if($row['cnt']){
            $result['status'] = 200;
            $result['data']['code'] = 2;
            $result['data']['msg'] = "이미 있는 아이디(이메일)입니다.";
            return $result;
        } else if(preg_match("/^[_\.0-9a-zA-Z-]+@([0-9a-zA-Z][0-9a-zA-Z-]+\.)+[a-zA-Z]{2,6}$/i", $reg_mb_id) == false) {
            $result['status'] = 200;
            $result['data']['code'] = 3;
            $result['data']['msg'] = "올바른 아이디(이메일)을 입력해주세요.";
            return $result;
        } 
        $result['status'] = 200;
        $result['data']['code'] = 4;
        $result['data']['msg'] = "사용하실 수 있는 아이디(이메일) 입니다";
        return $result;
    } catch(Exception $e){
        $result['status'] = 400;
        $result['data']['code'] = 5;
        $result['data']['msg'] = "에러가 발생했습니다";
        $result['data']['code'] = $e->getMessage();
        return $result;
    }
}