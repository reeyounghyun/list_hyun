...
if($board['bo_download_point'] < 0 && $is_guest)
    alert('다운로드 권한이 없습니다.\\n회원이시라면 로그인 후 이용해 보십시오.', G5_ADMIN_BBS_URL.'/login.php?wr_id='.$wr_id.'&amp;'.$qstr.'&amp;url='.urlencode(G5_ADMIN_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id));
...
if($js != 'on' && $board['bo_download_point'] < 0) {
    ...
    $url1 = G5_ADMIN_BBS_URL.'/download.php?'.clean_query_string($_SERVER['QUERY_STRING']).'&amp;js=on';
