...
nhn.husky.SE_EditingAreaManager = jindo.$Class({
	...
	// RawContents 대신 contents를 이용해도 되지만, content 획득을 위해서는 변환기를 실행해야 되기 때문에 RawContents 이용
...
*/
		...
		if(bSelectedBlock){
			...
			// 	-> Cannot monitor clicks/cursor position real-time so make the content error-proof instead.
			if(jindo.$Agent().navigator().firefox){
...
var oMessageMap_en_US = {
	...
	'SE_FindReplace.replaceAllResultP2' : ' matching content have been changed.',
	...
	'SE2M_EditingModeChanger.confirmTextMode' : 'The content remain, but editing effects, including fonts, and attachments, \n\nsuch as images, will disappear when changed to text mode. \n\n Make changes?',
...
//		var sBookmarkID = oSelection.placeStringBookmark();
		...
		// This is done to position the cursor correctly when the content of the blockquote is empty in Chrome.
...
nhn.FindReplace = jindo.$Class({
	...
	find : function(sKeyword, bCaseMatch, bBackwards, bWholeWord){
		...
		// end of the content could have been reached so search again from the beginning
		this.bEOC = true;
