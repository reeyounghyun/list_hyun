...
//        alert('로그인된 회원과 넘어온 정보가 서로 다릅니다.');
    ...
    */
    ...
    $mb_2 = basename($_FILES['mb_2']['name']);
    if($mb_2){
        if (is_uploaded_file($_FILES['mb_2']['tmp_name'])) {
            if ($_FILES['mb_2']['size'] > 100000000) {
                alert($_FILES['mb_2']['name'] . '의 크기가 10MB를 초과하였습니다.');
            }
            $img1 = $_FILES['mb_2']['name'];
            $img1 = explode('.', $img1);
        
        
            $img100 = $img1[0].".".$img1[1];
            @chmod(G5_DATA_PATH . '/member/', G5_DIR_PERMISSION);
            $dest_path = G5_DATA_PATH . '/member/' . $img100;
            move_uploaded_file($_FILES["mb_2"]["tmp_name"], $dest_path);
...
if($w=='u'){
    include_once($member_skin_path.'/register_renew_form.skin.php');
} else {
