...
if ($sca || $stx || $stx === '0') {     //검색이면
    ...
    */
    if($_GET['filter_search'] && $bo_table === 'item') {
        $ca_name = $_GET['sca'];
        isset($_GET['wr_8']) ? $wr_8 = $_GET['wr_8'] : $wr_8 = 'all';
        $sql_search = "";
        if ($wr_8 && $wr_8 !== 'all') {
            $sql_search .= "ca_name = '{$ca_name}' and wr_8 = '{$wr_8}'";
        }
        $sql = " select MIN(wr_num) as min_wr_num from {$write_table} ";
        $row = sql_fetch($sql);
        $min_spt = (int)$row['min_wr_num'];
...
} else if($_GET['filter_search'] && $bo_table === 'program'){
    isset($_GET['city']) ? $wr_8 = $_GET['city'] : $wr_8 = 'all';
    isset($_GET['district']) ? $wr_9 = $_GET['district'] : $wr_9 = 'all';
    isset($_GET['type']) ? $wr_10 = $_GET['type'] : $wr_10 = 'all';
    $sql_search = "";
    if($wr_8 && $wr_8 !=='all'){
        $sql_search .= " and wr_8 = '{$wr_8}'";
    }
    if($wr_9 && $wr_9 !=='all'){
        $sql_search .= " and wr_9 = '{$wr_9}'";
    }
    if($wr_10 && $wr_10 !=='all'){
        $sql_search .= " and wr_10 = '{$wr_10}'";
    }
    $sql = " select MIN(wr_num) as min_wr_num from {$write_table} ";
    $row = sql_fetch($sql);
    $min_spt = (int)$row['min_wr_num'];
...
if($_GET['filter_search'] && $bo_table === 'program'){
    isset($_GET['city']) ? $wr_8 = $_GET['city'] : $wr_8 = 'all';
    isset($_GET['district']) ? $wr_9 = $_GET['district'] : $wr_9 = 'all';
    isset($_GET['type']) ? $wr_10 = $_GET['type'] : $wr_10 = 'all';
    $sql_search = "";
...
if($_GET['filter_search'] && $bo_table === 'item'){
    isset($_GET['wr_8']) ? $wr_8 = $_GET['wr_8'] : $wr_8 = 'all';
    isset($_GET['sca']) ? $ca_name = $_GET['sca'] : $ca_name = '';
